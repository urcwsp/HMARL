...
class MultiHeadPPO:
    def __init__(self, state_dim, action_dim, action_std_init, lr, gamma, K_epochs, eps_clip, num_heads, device):
        ...
        self.MseLoss = nn.MSELoss()
        # Wasserstein正则化权重
        self.wasserstein_lambda = 0.2  # 可调整的超参数
        ...
        for _ in range(self.K_epochs):
            ...
            if self.num_heads > 1:  # 只有多头时才计算
                ...
                if wasserstein_distances:
                    # 平均Wasserstein距离，负号表示鼓励多样性（最大化距离）
                    wasserstein_reg = -torch.mean(torch.stack(wasserstein_distances))

            # 最终损失 = PPO损失 + Wasserstein正则化
            loss = ppo_loss + self.wasserstein_lambda * wasserstein_reg
