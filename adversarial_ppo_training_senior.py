...
def parse_args():
    ...
    parser.add_argument("--network_type", type=str, default="standard",
                        choices=["standard", "ltc", "deep", "wide"],
                        help="选择神经网络类型: standard(标准), ltc(液态时间常数), deep(深层), wide(宽层)")
...
def load_pretrained_protagonist(model_path, device):
    ...
    network_type = model_info.get('network_type', 'ltc')  # 默认为ltc保持向后兼容
    ...
    # 检查模型网络类型
    saved_network_type = model_info.get('network_type', 'ltc')  # 默认为ltc保持向后兼容
    current_network_type = network_type

    # 检查网络类型是否匹配
    if saved_network_type != current_network_type:
        print(f"警告: 预训练模型网络类型({saved_network_type})与当前指定的网络类型({current_network_type})不匹配。将重新初始化模型权重。")
    # 只有当网络类型匹配时才加载权重
        ...
        try:
            ...
            print(f"成功加载预训练{saved_network_type}模型权重")
...
class TrainingLogger:
    ...
    def log_stage1_data(self, epoch, params, protagonist_reward, episode_length, avg_step_duration=None):
        data = {
            'epoch': epoch,
    ...
    def log_stage2_data(self, epoch, params, protagonist_reward, episode_length, avg_step_duration=None):
        data = {
            'epoch': epoch,
            'episode_length': episode_length,
            'avg_step_duration': avg_step_duration,
    ...
    def log_stage3_data(self, epoch, params, protagonist_reward, adversary_reward, episode_length, alpha=0.7, action_diff=None, avg_step_duration=None, protagonist_action=None, adversary_action=None, mixed_action=None):
        data = {
            'epoch': epoch,
            'episode_length': episode_length,
            'avg_step_duration': avg_step_duration,
        ...
        # 添加主角动作（如果提供）
        if protagonist_action is not None:
            if isinstance(protagonist_action, np.ndarray):
                for i, val in enumerate(protagonist_action):
                    data[f'protagonist_action_{i}'] = val
            else:
                data['protagonist_action'] = protagonist_action

        # 添加对手动作（如果提供）
        if adversary_action is not None:
            if isinstance(adversary_action, np.ndarray):
                for i, val in enumerate(adversary_action):
                    data[f'adversary_action_{i}'] = val
            else:
                data['adversary_action'] = adversary_action

        # 添加混合动作（如果提供）
        if mixed_action is not None:
            if isinstance(mixed_action, np.ndarray):
                for i, val in enumerate(mixed_action):
                    data[f'mixed_action_{i}'] = val
            else:
                data['mixed_action'] = mixed_action
    ...
    def log_test_data(self, test_round, avg_reward, level_results=None, memory_info=None):
        """记录测试数据 - 保存测试轮次、整体平均奖励和各级别结果（包括成功率等统计信息）以及内存占用"""
        ...
        # 添加内存信息
        if memory_info:
            data['memory_used_mb'] = memory_info.get('used_mb', 0)
            data['memory_percent'] = memory_info.get('percent', 0)
            data['memory_available_mb'] = memory_info.get('available_mb', 0)

        # 添加各级别的测试结果（包括奖励和统计信息）
        if level_results:
            for level, result in level_results.items():
                # 如果result是字典（包含详细统计信息）
                if isinstance(result, dict):
                    data[f'{level}_reward'] = result.get('avg_reward', 0)
                    data[f'{level}_success_rate'] = result.get('success_rate', 0)
                    data[f'{level}_collision_rate'] = result.get('collision_rate', 0)
                    data[f'{level}_out_of_road_rate'] = result.get('out_of_road_rate', 0)
                    data[f'{level}_timeout_rate'] = result.get('timeout_rate', 0)
                # 如果result是数值（向后兼容）
                else:
                    data[f'{level}_reward'] = result
...
# 网络架构选择函数
def get_network_class(network_type, is_multihead=False):
    """
    根据网络类型返回对应的网络类

    Args:
        network_type: 网络类型 ("standard", "ltc", "deep", "wide")
        is_multihead: 是否为多头网络（对手网络）
    ...
    if is_multihead:
        if network_type == "ltc":
            return LTCMultiHeadActorCritic
        elif network_type == "deep":
            return DeepMultiHeadActorCritic
        elif network_type == "wide":
            return WideMultiHeadActorCritic
        else:  # standard
            return MultiHeadActorCritic
    else:
        if network_type == "ltc":
            return LTCActorCritic
        elif network_type == "deep":
            return DeepActorCritic
        elif network_type == "wide":
            return WideActorCritic
        else:  # standard
            return ActorCritic
...
class PPO:
    def __init__(self, state_dim, action_dim, action_std_init, lr, gamma, K_epochs, eps_clip, device, network_type="standard"):
        ...
        # 根据网络类型选择相应的网络架构
        network_class = get_network_class(network_type, is_multihead=False)
...
# 深层多头对手网络架构
class DeepMultiHeadActorCritic(nn.Module):
    def __init__(self, state_dim, action_dim, action_std_init, num_heads, device):
        super(DeepMultiHeadActorCritic, self).__init__()
        self.device = device
...
# 宽层多头对手网络架构
class WideMultiHeadActorCritic(nn.Module):
    def __init__(self, state_dim, action_dim, action_std_init, num_heads, device):
        super(WideMultiHeadActorCritic, self).__init__()
        self.device = device
        self.num_heads = num_heads
        ...
        # 多头值函数网络 - 每个对手一个头
        self.critic_heads = nn.ModuleList([
            nn.Linear(256, 1) for _ in range(num_heads)
        ])
...
# 多头对手PPO算法
class MultiHeadPPO:
    def __init__(self, state_dim, action_dim, action_std_init, lr, gamma, K_epochs, eps_clip, num_heads, device, network_type="standard"):
        self.gamma = gamma
        self.eps_clip = eps_clip
        self.K_epochs = K_epochs
        self.device = device
        ...
        # 根据网络类型选择相应的多头网络架构
        network_class = get_network_class(network_type, is_multihead=True)

        self.policy = network_class(state_dim, action_dim, action_std_init, num_heads, device).to(device)
        self.optimizer = torch.optim.Adam(self.policy.parameters(), lr=lr)

        self.policy_old = network_class(state_dim, action_dim, action_std_init, num_heads, device).to(device)
        self.policy_old.load_state_dict(self.policy.state_dict())
...
# 测试函数 - 每200回合测试一次，只测试主角（不加对手）
def run_test_episodes(protagonist, env, test_difficulty="total", num_test_episodes=100, test_levels=None):
    """
    运行测试回合，只测试主角（不加对手），每个难度级别测试固定回合数

    Args:
        protagonist: 主角智能体
        env: 环境
        test_difficulty: 基础测试难度（如果test_levels为None时使用）
        num_test_episodes: 每个级别的测试回合数（默认100回合）
        test_levels: 测试级别列表，如["easy", "medium", "hard"]，None表示只测试基础难度

    Returns:
        测试结果字典 {difficulty: {'avg_reward': float, 'success_rate': float, ...}}
    """
    # 保存当前环境配置
    ...
    for level in test_levels:
        print(f"  测试难度级别: {level} (测试{num_test_episodes}回合)")
        level_rewards = []
        level_config = get_param_config_by_difficulty(level)

        # 统计各种结果
        success_count = 0      # 成功到达目的地
        collision_count = 0    # 碰撞
        out_of_road_count = 0  # 脱轨/出界
        timeout_count = 0      # 超时

        # 对于每个难度级别，采样一次参数，然后用这个参数测试所有回合
        test_params = sample_random_params_from_config(level_config, level)

        # 更新环境配置为固定参数
        for param_name, param_val in test_params.items():
            env.config["random_dynamics"][param_name] = (param_val, param_val)

        # 测试 num_test_episodes 回合
        for episode in range(num_test_episodes):
            ...
            for t in range(env.config["horizon"]):
                # 主角选择动作（仅测试，不收集梯度，不使用对手）
                with torch.no_grad():
                    action, _ = protagonist.select_action(state)

                # 环境交互（只使用主角动作，不混合对手动作）
            ...
            if info.get('arrive_dest', False):
                ...
                # 如果既没有成功、也没有碰撞、也没有出界，则认为是超时
                timeout_count += 1

        # 计算当前级别的平均奖励和各种率
        level_avg_reward = np.mean(level_rewards)
        success_rate = (success_count / num_test_episodes) * 100
        collision_rate = (collision_count / num_test_episodes) * 100
        out_of_road_rate = (out_of_road_count / num_test_episodes) * 100
        timeout_rate = (timeout_count / num_test_episodes) * 100

        test_results[level] = {
            'avg_reward': level_avg_reward,
        ...
        print(f"    成功率: {success_rate:.1f}% | 碰撞率: {collision_rate:.1f}% | 脱轨率: {out_of_road_rate:.1f}% | 超时率: {timeout_rate:.1f}%")
...
# 三阶段训练函数
def three_stage_ppo_training(pretrained_model_path=None, skip_stage1=False, skip_stage2=False, initial_alpha=0.9, adv_num=1, network_type="standard", param_difficulty="total", stage1_difficulty="easy", stage2_difficulty="medium", stage3_difficulty="easy", test_difficulty="total", test_levels=None, test_episodes_per_level=50):
    # 设置随机种子
    seed = 42
    ...
    print(f"对手使用{network_names.get(network_type, network_type)}神经网络架构")
    ...
    if pretrained_model_path:
        ...
        try:
            if isinstance(protagonist.policy, LTCActorCritic):
                ...
                # 检查其他类型模型的输入输出维度
                if hasattr(protagonist.policy, 'actor') and hasattr(protagonist.policy.actor, '__getitem__'):
                    if protagonist.policy.actor[0].in_features != protagonist_state_dim or \
                       protagonist.policy.actor[-1].out_features != protagonist_action_dim:
                        raise ValueError(f"预训练模型维度与环境不匹配! 模型: {protagonist.policy.actor[0].in_features}->{protagonist.policy.actor[-1].out_features}, 环境: {protagonist_state_dim}->{protagonist_action_dim}")
                else:
                    # 对于其他网络结构，尝试推断输入输出维度
                    print("警告: 无法自动验证模型维度，请确保预训练模型与环境兼容")
        except Exception as e:
            print(f"模型维度验证时出错: {e}")
            print("将继续使用预训练模型，但可能存在兼容性问题")

        # 从模型信息中提取奖励历史（如果有）
        if model_info and 'rewards_history' in model_info:
            print(f"从预训练模型加载了 {len(model_info['rewards_history'])} 个历史奖励记录")
    else:
        # 创建新的主角智能体，根据network_type参数选择网络架构
        print(f"主角使用{network_names.get(network_type, network_type)}神经网络架构")
    ...
    if stage1_epochs > 0:
        ...
        while not stage1_completed:
            ...
            avg_step_duration = episode_duration / episode_length if episode_length > 0 else 0

            # 记录主角奖励
            protagonist_rewards.append(episode_reward)

            # 记录本轮数据到日志
            logger.log_stage1_data(current_epoch, current_params, episode_reward, episode_length, avg_step_duration)
            ...
            if current_epoch % 200 == 0:
                ...
                print("开始测试主角在不同难度参数下的表现（只测试主角，不加对手）...")
                ...
                # 记录测试数据 - 记录所有级别的平均值
                overall_avg = np.mean([result['avg_reward'] for result in test_results.values()])

                # 获取内存占用信息
                process = psutil.Process()
                memory_info_data = process.memory_info()
                virtual_memory = psutil.virtual_memory()
                memory_info = {
                    'used_mb': memory_info_data.rss / (1024 * 1024),  # 当前进程使用的内存（MB）
                    'percent': virtual_memory.percent,  # 系统内存使用百分比
                    'available_mb': virtual_memory.available / (1024 * 1024)  # 系统可用内存（MB）
                }

                # 传入完整的测试结果（包含统计信息）和内存信息
                logger.log_test_data(test_round, overall_avg, test_results, memory_info)

                print(f"测试结果总览: 整体平均奖励={overall_avg:.2f}")
                for level, result in test_results.items():
                    print(f"  {level}难度: 奖励={result['avg_reward']:.2f}, 成功率={result['success_rate']:.1f}%")
                print(f"内存占用: 进程={memory_info['used_mb']:.2f}MB, 系统={memory_info['percent']:.1f}%, 可用={memory_info['available_mb']:.2f}MB")
                print("=" * 50)
    ...
    if stage2_epochs > 0:
        ...
        while not stage2_completed:
            ...
            # 记录本轮数据到日志
            logger.log_stage2_data(current_epoch, random_params, episode_reward, episode_length, avg_step_duration)
    ...
    while not stage3_completed:
        ...
        for adv_idx, adversary in enumerate(adversaries):
            ...
            # 显示当前正在与哪个对手交互
            print(f"\n>>> 回合 {current_epoch} - 正在与对手 {adv_idx} 交互 <<<")
            ...
            # 用于记录最后一步的动作
            last_protagonist_action = None
            last_adversary_action = None
            last_mixed_action = None
            ...
            for t in range(env.config["horizon"]):
                ...
                # 保存最后一步的动作
                last_protagonist_action = protagonist_action.copy()
                last_adversary_action = adversary_action.copy()
                last_mixed_action = mixed_action.copy()
            ...
            # 记录本轮数据到日志，添加对手索引信息、alpha值和动作数据
            ...
            logger.log_stage3_data(current_epoch, params_with_adv, episode_reward, adversary_reward,
                                ...
                                avg_step_duration=avg_step_duration,
                                protagonist_action=last_protagonist_action,
                                adversary_action=last_adversary_action,
                                mixed_action=last_mixed_action)
            ...
            # 输出每轮的奖励，明确显示对手编号
            print(f"回合 {current_epoch:3d} | 对手 {adv_idx} | Alpha {current_alpha:.2f} | 主角奖励 {episode_reward:8.2f} | 对手奖励 {adversary_reward:8.2f}")
                ...
                if adv_rewards:  # 确保有奖励记录
                    ...
                    if adversary_active[adv_idx] and last_adv_avg < -1200:
                        ...
                        if adversary_alphas[adv_idx] <= 0.6:
                            ...
                            # 保存主角模型（包含对手编号）
                            protagonist_file = f"models/protagonist_vs_adv{adv_idx}_{save_timestamp}.pth"
                            protagonist_save_info = {
                                'policy_state_dict': protagonist.policy.state_dict(),
                                'lr': protagonist.policy.optimizer.param_groups[0]['lr'] if hasattr(protagonist.policy, 'optimizer') else 0.0003,
                                'gamma': protagonist.gamma,
                                'K_epochs': protagonist.K_epochs,
                                'eps_clip': protagonist.eps_clip,
                                'network_type': network_type,
                                'adversary_idx': adv_idx,  # 记录对应的对手编号
                                'adversary_alpha': adversary_alphas[adv_idx],  # 记录对手的alpha值
                                'current_epoch': current_epoch,
                                'save_reason': f'adversary_{adv_idx}_removed'
                            }

                            # 如果是LTC模型，添加LTC特定参数
                            if isinstance(protagonist.policy, LTCActorCritic):
                                protagonist_save_info['ltc_config'] = {
                            ...
                            torch.save(protagonist_save_info, protagonist_file, pickle_protocol=4)
                            print(f"已保存主角模型至: {protagonist_file}")

                            # 保存对手模型（单个头的信息）
                            adversary_file = f"models/adversary_{adv_idx}_{save_timestamp}.json"
                            adversary_save_info = {
                                'head_idx': adv_idx,
                                'alpha': adversary_alphas[adv_idx],
                                'active': adversary_active[adv_idx],
                                'current_epoch': current_epoch,
                                'multi_head_ppo_file': f"models/multi_head_ppo_adv{adv_idx}_{save_timestamp}.pth"
                            }

                            # 保存多头PPO模型的当前状态
                            multi_head_file = adversary_save_info['multi_head_ppo_file']
                            multi_head_save_info = {
                                'policy_state_dict': multi_head_ppo.policy.state_dict(),
                                'num_heads': adv_num,
                                'alphas': adversary_alphas.copy(),
                                'active': adversary_active.copy(),
                                'network_type': network_type,
                                'saved_for_adversary': adv_idx
                            }

                            # 如果是LTC模型，添加LTC特定参数
                            if isinstance(multi_head_ppo.policy, LTCMultiHeadActorCritic):
                                multi_head_save_info['ltc_config'] = {
                            ...
                            torch.save(multi_head_save_info, multi_head_file)
                            print(f"已保存多头对手模型至: {multi_head_file}")

                            with open(adversary_file, 'w') as f:
                                json.dump(adversary_save_info, f, indent=4)
    ...
    protagonist_info = {
        ...
        'policy_state_dict': protagonist.policy.state_dict(),
        'network_type': network_type,  # 标记网络类型
        ...
        'training_config': {
            ...
            'network_type': network_type,  # 添加网络类型
        },
